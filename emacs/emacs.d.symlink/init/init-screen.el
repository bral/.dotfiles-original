...
(defface elscreen-tab-control-face
  '((((type x w32 mac) (class color))
     (:background "#808080" :foreground "#585858" :underline "#585858"))
    (((class color))
     (:background "#93A1A1" :foreground "black" :underline t))
    (t (:underline t)))
  "Face for control tab."
  :group 'elscreen)

(defvar solblue "#0087ff")

(defface elscreen-tab-current-screen-face
  '((((class color))
     (:background "#585858" :foreground "#e4e4e4"))
    (t (:underline t)))
...
(defface elscreen-tab-other-screen-face
  '((((type x w32 mac) (class color))
     :background "Gray85" :foreground "#93A1A1")
    (((class color))
...
(global-set-key (caps-lock-bind "t") 'elscreen-create)
(global-set-key (caps-lock-bind "}") 'elscreen-next)
(global-set-key (caps-lock-bind "{") 'elscreen-previous)
(global-set-key (caps-lock-bind "w")
  (λ (if (elscreen-one-screen-p)
         (delete-frame)
         (elscreen-kill))))
